...
const initializeChat = (server: SocketIOServer): void => {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on(
      "message",
      async (room: string, message: string, senderId: string) => {
        // Save the chat message to the database
        try {
          const team = await TeamModel.findOne({ _id: room }).populate(
            "chat.sender"
          );
          ...
          const isValidSender =
            team._user.toString() === senderId ||
            team.applicants.some(
              (applicant) => applicant._user.toString() === senderId
            );
          if (!isValidSender) {
